...
package com.cloudera.datascience.intro.tweaked
...
object RunIntro extends Serializable {
  ...
  case class Args(dataPath: String)

  def parseArgs(args: Array[String]): Args = {
    if(args.isEmpty) {
      println("You must provide a file path to the data")
      sys.exit(-1)
    }
    Args(args(0))
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawblocks = sc.textFile(parsedArgs.dataPath)
